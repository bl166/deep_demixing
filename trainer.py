...
class _trainer(object):
    ...
    def logging(self, info, phase):
        ...
        if phase == 'train':
            ...
            for k,v in info.items():
                self.logger.add_scalar('%s/val'%k, v, self.epoch)
...
# ----------------------------------------------
# ---- Graph CAVE Trainer (classification) ----
# ----------------------------------------------
    ...
    def predict(self, epoch, criterion=None, loader=None, save=False):
        ...
        with torch.no_grad():
            ...
            if criterion:
                ...
                if save: # if validation, save the model if resulting in a new lowest error/loss
                    self.logging(info, 'val')
