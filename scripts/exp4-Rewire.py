...
from models import *#CVAE_UNET_Batch, MLP_benchmark, CNN_time_benchmark, CNN_nodes_benchmark, GRU_benchmark, LSTM_benchmark, LSTM_bi_benchmark, CNN_time2_benchmark
...
parser.add_argument('--topology',    type=str, default='SW')
parser.add_argument('--observation', type=str, default='SIRS')
parser.add_argument('--probability', type=float, default=0.005)
...
if __name__ == '__main__':
    ...
    learning_rate = 1e-2  # learning rate (old values: 1e-2 for CVAE, 1e-4 for RNN, 1e-3 for other benchmarks)
    ...
    graph_param,_ = set_params(g_type, n=N, p=prob)
    ...
    fin = len(gt_type.split('+'))
    models = [
        CNN_time2_benchmark(T, N, feat_in=fin),
        CNN_nodes_benchmark(T, N, feat_in=fin),
        MLP_benchmark(T, N, feat_in=fin),
        LSTM_bi_benchmark(T, N, feat_in=fin, depth=2),
        CVAE_UNET_Batch(encoder_feat, posterior_feat, T, N, encoder_feat_in=fin, prior_feat_in=fin),
    ]
    ...
    for MODEL in models: 
        ...
        TRAINER = GCVAE_Trainer if "CVAE_UNET" in m_str else Benchmark_Trainer 
        ...
        save_path = PROJ_RT + "/__models_rewire/seed_{rd}/run_{run}/".format(rd = rand, run = run) \
            + "{ms:s}_{topo:s}_{g_param:s}_{ob:s}_{gt:s}_".format(ms=m_str, topo=g_type, g_param=str(graph_param), ob=ob_type, gt=gt_type) \
