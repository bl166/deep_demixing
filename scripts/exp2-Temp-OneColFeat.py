...
from models import CVAE_UNET_Batch, MLP_benchmark, CNN_time_benchmark, CNN_nodes_benchmark, GRU_benchmark, LSTM_benchmark, LSTM_bi_benchmark
...
if __name__ == '__main__':
    ...
    auto_stop     = 10    # terminate training if val loss stops going down for this many epochs
    ...
    models = [
        CNN_time_benchmark(T, N, feat_in=infeats),
        CNN_nodes_benchmark(T, N, feat_in=infeats),
        MLP_benchmark(T, N, feat_in=infeats),
        LSTM_benchmark(T, N, feat_in=infeats, depth=2, hidden_size=encoder_feat),
        CVAE_UNET_Batch(encoder_feat, posterior_feat, T, N, encoder_feat_in=infeats, prior_feat_in=infeats),
    ]
    ...
    for MODEL in models: 
        ...
        optimizer = torch.optim.Adam(model.parameters(), 
                                     ...
                                     weight_decay = l2   # l2 regularization coefficient 
                                    )
        # set the folder path to save models, checkpoints, and training logs 
        save_path = PROJ_RT + "/models_j/test2/seed_{rd}/run_{run}/".format(rd = rand, run = run) \
